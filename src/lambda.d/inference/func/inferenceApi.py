...
def insert_new_transaction_vertex_and_edge(tr_dict, connectted_node_dict, target_id, vertex_type = 'Transaction'):
    ...
    def insert_attr(attr_val_dict, target_id, node_id, vertex_type): 
        def insert_edge(target_id, node_id):
    ...
    for node_k, node_v in connectted_node_dict[0].items():
        ...
        insert_attr([empty_node_dict], target_id, node_id, vertex_type = node_k)
                           
                  
def query_target_subgraph(target_id, neighbor_cols, dummied_col):
...
def handler(event, context):
    ...
    insert_new_transaction_vertex_and_edge(trans_dict, identity_dict , target_id, vertex_type = 'Transaction')
    ...
    subgraph_dict, transaction_embed_value_dict = query_target_subgraph(target_id, neighbor_cols, dummied_col)
