...
spark_dataframe = spark_dataframe.select(spark_dataframe.TransactionID, to_json(struct(list(filter(lambda x: (x != TRANSACTION_ID), spark_dataframe.schema.names)))).alias('props_values:String'))
...
dump_df_to_s3_for_bulk_load(features_dynamic_df, f'features_dynamic_df') #.toDF().toPandas(), f'features_dynamic_df')
...
dump_df_to_s3_for_bulk_load(labels_dynamic_df, f'labels_dynamic_df') #.toDF().toPandas(), f'labels_dynamic_df')
